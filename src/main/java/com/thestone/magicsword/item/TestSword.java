...
public class TestSword extends SwordItem {
    public TestSword(ToolMaterial toolMaterial, int attackDamage, float attackSpeed, Settings settings) {
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        if (!user.getEntityWorld().isClient() && user instanceof PlayerEntity player) {
            ...
            if (remainingUseTicks < 11) {
                ...
                Helper.spawnDirectionalParticles((ServerWorld) world, ParticleTypes.SONIC_BOOM, player, 10, distance);
                Helper.damageEntitiesInTrajectory((ServerWorld) world, player, distance, damage, source);
